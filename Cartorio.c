...
int registro() //Função responsável por cadastrar os usuários no sistema
{
	...
	file = fopen(arquivo, "w"); //abre o arquivo e o "W" significa write(escrever)
...
int consulta()
{
	...
	char cpf[40];
	char conteudo[200];
	//fi criação de variáveis/string
	
	printf("Digite o CPF a ser consultado: "); //consultando as informações do usuário
	scanf("%s", cpf);
	
	FILE *file;
	file = fopen(cpf, "r"); //abre o arquivo e o "r" significa read(ler)
...
int deletar()
{
	...
	remove(cpf); //deleta o arquivo salvo
